...
def model_from_config(config) -> models.SelfSupervisedModel:
  ...
  kwargs = {
      ...
      "use_action": config.model.use_action,
...
def dataset_from_config(config, downstream, split, debug, k_fold=False):
  ...
  if debug:
    ...
    if k_fold:
      aug_names = config.k_fold.k_fold_transforms
    elif split == "train":
