...
class VideoDataset(Dataset):
  ...
  def __init__(
    ...
    if self._use_action or self._use_state:
      self._labels = np.load(
        osp.join(root_dir, task, "labels.npy"),
        allow_pickle=True
      ).item()
  ...
  def _get_data(self, vid_path):
    ...
    vid_num = int(osp.basename(vid_path))
    if self._use_action or self._use_state:
      cumm_len = np.sum(
        [self._labels['seq_lens'][i] for i in range(vid_num)],
        dtype=int
      )
    if self._use_action:
      actions = self._labels['actions'][cumm_len + frame_idxs, :]
      data[SequenceType.ACTION] = actions
    if self._use_state:
      states = self._labels['states'][cumm_len + frame_idxs, :]
      data[SequenceType.STATE] = states
  ...
  def collate_fn(
    ...
    if self._use_action:
      data[str(SequenceType.ACTION)] = _stack(SequenceType.ACTION)
    if self._use_state:
      data[str(SequenceType.STATE)] = _stack(SequenceType.STATE)
