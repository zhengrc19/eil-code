...
class SelfSupervisedModel(abc.ABC, nn.Module):
  ...
  def forward(self, x):
    ...
    feats = self.backbone(x_flat)   # B * F * CTX, c, h, w
    _, c, h, w = feats.shape            # want B*F, c, CTX, h, w
    feats = feats.view((-1, self.num_ctx_frames, c, h, w)) # B*F, CTX, c, h, w
    feats = feats.permute((0, 2, 1, 3, 4))  # B*F, c, CTX, h, w
    feats = self.temporal_conv(feats)
    feats_flat = torch.flatten(feats, 1)
    feats_flat = self.mlp(feats_flat)

    if self.use_action:
      actions = F.normalize(actions, dim=-1)
      action_feats = self.action_encoder(actions)
      action_feats_flat = torch.flatten(action_feats, 0, 1)
      feats_flat = torch.cat([feats_flat, action_feats_flat], 1)
    if self.use_state:
      states = F.normalize(states, dim=-1)
      state_feats = self.state_encoder(states)
      state_feats_flat = torch.flatten(state_feats, 0, 1)
      feats_flat = torch.cat([feats_flat, state_feats_flat], 1)
    ...
    embs = embs.view((batch_size, t // self.num_ctx_frames, -1))
    feats = feats.view((batch_size, t // self.num_ctx_frames, -1))
  ...
  def infer(
    ...
    if self.use_state:
      states = x['state']
    if frames.shape[1] > effective_bs:
      out = []
      inp = {}
      for i in range(math.ceil(frames.shape[1] / effective_bs)):
        inp['frames'] = frames[:, i * effective_bs:(i + 1) * effective_bs]
        if self.use_action:
          inp['action'] = actions[
            :,
            i * int(effective_bs / self.num_ctx_frames):
            (i + 1) * int(effective_bs / self.num_ctx_frames)
          ]
        if self.use_state:
          inp['state'] = states[
            :,
        ...
        out.append(self.forward(inp).cpu())
...
class Resnet18LinearEncoderNet(SelfSupervisedModel):
  ...
  def __init__(self, embedding_size, *args, **kwargs):
    ...
    layers_ = list(resnet.children())[:-2]
    self.backbone = nn.Sequential(*layers_)
    self.temporal_conv = nn.Sequential(
      nn.Conv3d(num_ftrs, num_ftrs, 3, padding=1),
      nn.Conv3d(num_ftrs, num_ftrs, 3, padding=1),
    ...
    # Encoder.
    self.encoder = nn.Linear(encoder_in_ftrs, embedding_size)
