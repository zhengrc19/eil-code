...
def get_downstream_dataloaders(
  ...
  def _loader(split, k_fold=False):
    datasets = factory.dataset_from_config(config, True, split, debug, k_fold)
    loaders = {}
  ...
  return {
      "train": _loader("train"),
      "valid": _loader("valid"),
      "k_fold": _loader("train", k_fold=True)
  }
...
def get_factories(
  ...
  downstream_loaders = get_downstream_dataloaders(config, debug=True)
