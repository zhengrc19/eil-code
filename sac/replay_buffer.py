...
class ReplayBufferLearnedReward(abc.ABC, ReplayBuffer):
  ...
  def __init__(
    ...
    if stride is not None:
      self.pixels_buffer = deque(maxlen=stride + 1)
    self._reset_staging()
  ...
  def insert(
    ...
    if len(self.obses_staging) < self.batch_size:
      ...
      if self.stride is not None:
        if not mask:
          self.pixels_buffer = deque(maxlen=self.stride + 1)
        self.pixels_buffer.append(pixels)
        self.pixels_staging.append(self.pixels_buffer[0])
    else:
...
class ReplayBufferDistanceToGoal(ReplayBufferLearnedReward):
  ...
  def _get_reward_from_image(self):
    ...
    if self.model.use_action:
      actions = torch.from_numpy(np.stack(self.actions_staging)).float().to(self.device).unsqueeze(0)
      batch['action'] = actions
    if self.model.use_state:
      states = torch.from_numpy(np.stack(self.obses_staging)).float().to(self.device).unsqueeze(0)
      batch['state'] = states
    embs = self.model.infer(batch).numpy().embs
